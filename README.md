...
## Instructions
* The algorithm and get the result you must run the **project.py**.
  * Before running project.py it requires **filling the Csv_Data folder with valid data**.
  * To fill the folder we created a Colab Notebook, that takes image data from Kaggle API and performs pre-processing on the data.
  * The link is located below or here: 
    ...
    * We have taken 5 large datasets from Kaggle API which were pre-processed to 10 different datasets (the other datasets were taken from Keras API).   
